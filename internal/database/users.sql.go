...
const markUserRed = `-- name: MarkUserRed :one
UPDATE users
...
func (q *Queries) MarkUserRed(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, markUserRed, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
